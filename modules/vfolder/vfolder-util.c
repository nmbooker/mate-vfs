...
static void
monitor_callback_internal (MateVFSMonitorHandle *handle,
...
monitor_timeout_cb (gpointer user_data)
{
	...
	for (iter = copy; iter; iter = iter->next) {
		...
		(*monitor->callback) ((MateVFSMonitorHandle *) monitor,
				      monitor->uri,
				      monitor->uri,
				      ctime == 0 ?
...
monitor_start_internal (MateVFSMonitorType      type,
			const gchar             *uri,
...
vfolder_monitor_dir_new (const gchar             *uri,
			 ...
			 gpointer                 user_data)
{
	return monitor_start_internal (MATE_VFS_MONITOR_DIRECTORY, 
				       uri, 
				       callback,
				       user_data);
}
...
vfolder_monitor_file_new (const gchar             *uri,
			  ...
			  gpointer                 user_data)
{
	return monitor_start_internal (MATE_VFS_MONITOR_FILE, 
				       uri, 
				       callback,
				       user_data);
}
...
make_directory_and_parents_from_uri (MateVFSURI *uri, guint permissions)
{
	MateVFSResult result;
