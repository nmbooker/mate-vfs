...
static void itemdir_monitor_cb (MateVFSMonitorHandle    *handle,
				const gchar              *monitor_uri,
				const gchar              *info_uri,
				MateVFSMonitorEventType  event_type,
				gpointer                  user_data);

static void writedir_monitor_cb (MateVFSMonitorHandle    *handle,
				 const gchar              *monitor_uri,
				 const gchar              *info_uri,
				 MateVFSMonitorEventType  event_type,
				 gpointer                  user_data);

static void desktopdir_monitor_cb (MateVFSMonitorHandle    *handle,
				   const gchar              *monitor_uri,
				   const gchar              *info_uri,
				   MateVFSMonitorEventType  event_type,
				   gpointer                  user_data);
...
static void
itemdir_monitor_cb (MateVFSMonitorHandle    *handle,
		    const gchar              *monitor_uri,
		    const gchar              *info_uri,
		    MateVFSMonitorEventType  event_type,
		    gpointer                  user_data)
{
	...
	D (g_print ("*** Itemdir '%s' monitor %s%s%s called! ***\n", 
		    info_uri,
		    event_type == MATE_VFS_MONITOR_EVENT_CREATED ? "CREATED":"",
...
static void 
writedir_monitor_cb (MateVFSMonitorHandle    *handle,
		     const gchar              *monitor_uri,
		     const gchar              *info_uri,
		     MateVFSMonitorEventType  event_type,
		     gpointer                  user_data)
...
static void 
desktopdir_monitor_cb (MateVFSMonitorHandle    *handle,
		       const gchar              *monitor_uri,
		       const gchar              *info_uri,
		       MateVFSMonitorEventType  event_type,
		       gpointer                  user_data)
...
filename_monitor_handle (gpointer user_data)
{
	...
	for (iter = info->requested_monitors; iter; iter = iter->next) {
		...
		if (mhandle->type == MATE_VFS_MONITOR_DIRECTORY) {
			folder = 
...
static void
filename_monitor_cb (MateVFSMonitorHandle *handle,
		     const gchar *monitor_uri,
		     const gchar *info_uri,
		     MateVFSMonitorEventType event_type,
		     gpointer user_data)
...
vfolder_info_find_filenames (VFolderInfo *info)
{
	 ...
	 * /etc/mate-vfs-2.0/vfolders/scheme.vfolder-info.  Writability will
...
vfolder_info_init (VFolderInfo *info)
{
	...
	if (g_getenv ("MATE_VFS_VFOLDER_INFODIR")) {
		...
		info->filename = 
			vfolder_build_uri (
				g_getenv ("MATE_VFS_VFOLDER_INFODIR"),
				filename,
				NULL);
		g_free (filename);
	}

	if (g_getenv ("MATE_VFS_VFOLDER_WRITEDIR")) {
		g_free (info->write_dir);
		info->write_dir = 
			vfolder_build_uri (
				g_getenv ("MATE_VFS_VFOLDER_WRITEDIR"),
				info->scheme,
				NULL);
	}
...
vfolder_info_destroy (VFolderInfo *info)
{
	...
	while (info->requested_monitors) {
		MateVFSMethodHandle *monitor = info->requested_monitors->data;
...
void 
vfolder_info_cancel_monitor (MateVFSMethodHandle  *handle)
{
