...
{
	MateVFSHandle *handle;
	MateVFSDirectoryHandle *dhandle;
	MateVFSResult result;
	gchar *uri, *realuri, *content;
	MateVFSFileSize writelen;
	...
	realuri = g_build_filename (getenv ("MATE_VFS_VFOLDER_WRITEDIR"),
				    "test-vfolder",
	...
	result = mate_vfs_create (&handle, 
				   ...
				   FALSE,
				   MATE_VFS_PERM_USER_ALL);
	TEST_RESULT (result != MATE_VFS_OK, "ERROR CREATING FILE IN WRITEDIR");
...
main (int argc, char **argv)
{
	...
	putenv (g_strconcat ("MATE_VFS_VFOLDER_INFODIR=", cwd, NULL));
